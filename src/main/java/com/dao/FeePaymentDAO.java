  
  package com.dao;

import java.sql.*;
import java.util.*;
import com.model.FeePayment;

public class FeePaymentDAO {

    private Connection getConnection() throws Exception {

        Class.forName("com.mysql.cj.jdbc.Driver");

        return DriverManager.getConnection(
            "jdbc:mysql://localhost:3306/javaproject",
            "root",
            "yourpassword"
        );
    }

    // ADD PAYMENT
    public void addPayment(FeePayment f) throws Exception {

        Connection con = getConnection();

        String sql =
            "INSERT INTO FeePayments(StudentID, StudentName, PaymentDate, Amount, Status) VALUES(?,?,?,?,?)";

        PreparedStatement ps = con.prepareStatement(sql);

        ps.setInt(1, f.getStudentID());
        ps.setString(2, f.getStudentName());
        ps.setString(3, f.getPaymentDate());
        ps.setDouble(4, f.getAmount());
        ps.setString(5, f.getStatus());

        ps.executeUpdate();
        con.close();
    }

    // UPDATE PAYMENT
    public void updatePayment(FeePayment f) throws Exception {

        Connection con = getConnection();

        String sql =
            "UPDATE FeePayments SET StudentID=?, StudentName=?, PaymentDate=?, Amount=?, Status=? WHERE PaymentID=?";

        PreparedStatement ps = con.prepareStatement(sql);

        ps.setInt(1, f.getStudentID());
        ps.setString(2, f.getStudentName());
        ps.setString(3, f.getPaymentDate());
        ps.setDouble(4, f.getAmount());
        ps.setString(5, f.getStatus());
        ps.setInt(6, f.getPaymentID());

        ps.executeUpdate();
        con.close();
    }

    // DELETE PAYMENT
    public void deletePayment(int id) throws Exception {

        Connection con = getConnection();

        PreparedStatement ps =
            con.prepareStatement("DELETE FROM FeePayments WHERE paymentID=?");

        ps.setInt(1, id);

        ps.executeUpdate();
        con.close();
    }

    // VIEW ALL PAYMENTS
    public List<FeePayment> getAllPayments() throws Exception {

        List<FeePayment> list = new ArrayList<>();

        Connection con = getConnection();

        PreparedStatement ps = con.prepareStatement(
            "SELECT * FROM FeePayments " +
            "WHERE StudentName IS NOT NULL " +
            "AND Status IS NOT NULL " +
            "AND Amount > 0"
        );

        ResultSet rs = ps.executeQuery();

        while(rs.next()) {

            FeePayment f = new FeePayment();

            f.setPaymentID(rs.getInt("paymentID"));
            f.setStudentID(rs.getInt("StudentID"));
            f.setStudentName(rs.getString("StudentName"));
            f.setPaymentDate(rs.getString("PaymentDate"));
            f.setAmount(rs.getDouble("Amount"));
            f.setStatus(rs.getString("Status"));

            list.add(f);
        }

        con.close();
        return list;
    }

    // OVERDUE REPORT
    public List<FeePayment> getOverdueStudents() throws Exception {

        List<FeePayment> list = new ArrayList<>();

        Connection con = getConnection();

        PreparedStatement ps =
            con.prepareStatement(
                "SELECT * FROM FeePayments WHERE Status='Overdue' ORDER BY paymentID"
            );

        ResultSet rs = ps.executeQuery();

        while(rs.next()) {

            FeePayment f = new FeePayment();

            f.setPaymentID(rs.getInt("paymentID"));
            f.setStudentID(rs.getInt("StudentID"));
            f.setStudentName(rs.getString("StudentName"));
            f.setPaymentDate(rs.getString("PaymentDate"));
            f.setAmount(rs.getDouble("Amount"));
            f.setStatus(rs.getString("Status"));

            list.add(f);
        }

        con.close();
        return list;
    }

    // UNPAID REPORT
    public List<FeePayment> getUnpaidStudentsByDate(String from, String to) throws Exception {

        List<FeePayment> list = new ArrayList<>();

        Connection con = getConnection();

        PreparedStatement ps =
            con.prepareStatement(
                "SELECT * FROM FeePayments " +
                "WHERE PaymentDate BETWEEN ? AND ? " +
                "AND Status='Unpaid' ORDER BY paymentID"
            );

        ps.setString(1, from);
        ps.setString(2, to);

        ResultSet rs = ps.executeQuery();

        while(rs.next()) {

            FeePayment f = new FeePayment();

            f.setPaymentID(rs.getInt("paymentID"));
            f.setStudentID(rs.getInt("StudentID"));
            f.setStudentName(rs.getString("StudentName"));
            f.setPaymentDate(rs.getString("PaymentDate"));
            f.setAmount(rs.getDouble("Amount"));
            f.setStatus(rs.getString("Status"));

            list.add(f);
        }

        con.close();
        return list;
    }

    // TOTAL COLLECTION
    public double getTotalCollection() throws Exception {

        Connection con = getConnection();

        PreparedStatement ps =
            con.prepareStatement(
                "SELECT SUM(Amount) FROM FeePayments WHERE Status='Paid'"
            );

        ResultSet rs = ps.executeQuery();

        double total = 0;

        if(rs.next()) {
            total = rs.getDouble(1);
        }

        con.close();
        return total;
    }

    // FETCH BY ID
    public FeePayment getPaymentByID(int id) throws Exception {

        FeePayment f = null;

        Connection con = getConnection();

        PreparedStatement ps =
            con.prepareStatement(
                "SELECT * FROM FeePayments WHERE paymentID=?"
            );

        ps.setInt(1, id);

        ResultSet rs = ps.executeQuery();

        if(rs.next()) {

            f = new FeePayment();

            f.setPaymentID(rs.getInt("paymentID"));
            f.setStudentID(rs.getInt("StudentID"));
            f.setStudentName(rs.getString("StudentName"));
            f.setPaymentDate(rs.getString("PaymentDate"));
            f.setAmount(rs.getDouble("Amount"));
            f.setStatus(rs.getString("Status"));
        }

        con.close();
        return f;
    }

    // STUDENT NAME (⚠️ BETTER FIX NEEDED - SEE NOTE BELOW)
    public String getStudentNameById(int id) throws Exception {

        Connection con = getConnection();

        PreparedStatement ps =
            con.prepareStatement(
                "SELECT StudentName FROM FeePayments WHERE StudentID=?"
            );

        ps.setInt(1, id);

        ResultSet rs = ps.executeQuery();

        String name = "";

        if(rs.next()) {
            name = rs.getString("StudentName");
        }

        con.close();
        return name;
    }

    // ⭐ NEW METHOD (ADD PAYMENT ID AUTO GENERATION)
    public int getNextPaymentId() {

        int id = 1;

        try {

            Connection con = getConnection();

            String sql = "SELECT MAX(paymentID) FROM FeePayments";

            PreparedStatement ps = con.prepareStatement(sql);

            ResultSet rs = ps.executeQuery();

            if(rs.next()) {
                id = rs.getInt(1) + 1;
            }

            con.close();

        } catch(Exception e) {
            e.printStackTrace();
        }

        return id;
    }
}
